...
//	static uint8_t _USART2_DMA_RX_BUF[2][BSP_USART2_DMA_RX_BUF_LEN];//双缓冲接收区
	static uint8_t _USART2_DMA_RX_BUF[BSP_USART2_DMA_RX_BUF_LEN];
...
#if EN_USART3_DMA_SECOND_FIFO == 1
  DMA_InitStructure.DMA_Memory0BaseAddr   =   (uint32_t)&_USART3_DMA_RX_BUF[0][0];
  DMA_InitStructure.DMA_BufferSize = BSP_USART3_DMA_RX_BUF_LEN/2;
#else
  DMA_InitStructure.DMA_Memory0BaseAddr =(uint32_t)_USART3_RX_BUF ;     //DMA接收基地址
  DMA_InitStructure.DMA_BufferSize = BSP_USART3_DMA_RX_BUF_LEN ;        //传输数据数量
#endif	
  DMA_InitStructure.DMA_FIFOMode = DMA_FIFOMode_Disable ;
  DMA_InitStructure.DMA_FIFOThreshold = DMA_FIFOThreshold_1QuarterFull ;
  DMA_InitStructure.DMA_MemoryBurst = DMA_MemoryBurst_Single ;
...
{
	uint32_t this_time_rx_len3=0;
	if(USART_GetITStatus(USART_CH100, USART_IT_IDLE)!= RESET)//
	{
		  USART_ReceiveData(USART_CH100); //一定要读一次，不然可能会丢第一个字节，原因未知
#if EN_USART3_DMA_SECOND_FIFO == 1
      if(DMA_GetCurrentMemoryTarget(USART_CH100_RX_DMA_STREAM) == 0)
        {
          DMA_Cmd(USART_CH100_RX_DMA_STREAM, DISABLE);
          DMA_ClearFlag(USART_CH100_RX_DMA_STREAM, DMA_FLAG_TCIF1 | DMA_FLAG_HTIF1);
          this_time_rx_len3 = BSP_UART5_DMA_RX_BUF_LEN - DMA_GetCurrDataCounter(USART_CH100_RX_DMA_STREAM);

          DMA_SetCurrDataCounter(USART_CH100_RX_DMA_STREAM, BSP_USART3_DMA_RX_BUF_LEN);
          DMA_MemoryTargetConfig (USART_CH100_RX_DMA_STREAM,(uint32_t)&_USART3_DMA_RX_BUF[1][0],DMA_Memory_1);
					USART_CH100_RX_DMA_STREAM->CR |= (uint32_t)(DMA_SxCR_CT);                  //enable the current selected memory is Memory 1
          DMA_Cmd(USART_CH100_RX_DMA_STREAM, ENABLE);
      ...
      //Target is Memory1
      else
        {
          ...
          this_time_rx_len3 =BSP_USART3_DMA_RX_BUF_LEN - DMA_GetCurrDataCounter(USART_CH100_RX_DMA_STREAM);				

          DMA_SetCurrDataCounter(USART_CH100_RX_DMA_STREAM, BSP_USART3_DMA_RX_BUF_LEN);
          DMA_MemoryTargetConfig (USART_CH100_RX_DMA_STREAM,(uint32_t)&_USART3_DMA_RX_BUF[0][0],DMA_Memory_0);
					USART_CH100_RX_DMA_STREAM->CR |= (uint32_t)(DMA_SxCR_CT);                  //enable the current selected memory is Memory 1
          DMA_Cmd(USART_CH100_RX_DMA_STREAM, ENABLE);
