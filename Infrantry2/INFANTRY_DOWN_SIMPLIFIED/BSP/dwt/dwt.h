...
#define DWT_START_CNT(assist_cnt)   do{assist_cnt = DWT->CYCCNT/168000000*1000;}while(0);
#define DWT_END_CNT(assist_cnt)     (DWT->CYCCNT/168000000*1000-assist_cnt)
...
#define DELAY_EXE(control_cnt,Delay_ms,statements) do{EXE_ONCE(control_cnt,DWT_START_CNT(control_cnt)) \
																														if(DWT_END_CNT(control_cnt)<=(Delay_ms)){statements} \
																														}while(0);
