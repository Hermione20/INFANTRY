...
float pitch_max = 0;		
    ...
    #define YAW_INIT_ANGLE_FDB          -OT_yaw_Encoder.OT_Pos   //步兵机械将电机反着装导致yaw轴电机向右编码器角度为负，与期望极性相反，需要加负号
...
void gimbal_parameter_Init(void)
{
    ...
    //------------------------------------------------
    PID_struct_init(&gimbal_data.pid_init_yaw_Angle, POSITION_PID, 600, 10,
                    10, 0.1f, 20); 
    PID_struct_init(&gimbal_data.pid_init_yaw_speed, POSITION_PID, 4, 0,
                    0.15f, 0.8f, 0); 
...
//    PID_struct_init(&gimbal_data.pid_yaw_speed, POSITION_PID, 29000, 10000,
//                    420, 0.5f, 0); 
	PID_struct_init(&gimbal_data.pid_yaw_Angle, POSITION_PID, 500, 10,
                    4.8f, 0.0f, 0);
    ...
    PID_struct_init ( &gimbal_data.pid_yaw_follow, POSITION_PID,  4,0,
                       0.1, 0, 0);//15 0 80
    PID_struct_init ( &gimbal_data.pid_yaw_speed_follow, POSITION_PID, 4, 0,
                       0.1, 0, 0 ); //160 0.8 40
    ...
    PID_struct_init(&gimbal_data.pid_rotate_pit_speed, POSITION_PID, 25000, 2000,
                    180, 0.001f, 0); 
	  PID_struct_init(&gimbal_data.pid_rotate_yaw_Angle, POSITION_PID, 4, 0,
                    0, 0.0f, 0);
    PID_struct_init(&gimbal_data.pid_rotate_yaw_speed, POSITION_PID, 4, 0,
                    0, 0, 0);
    ...
    PID_struct_init(&gimbal_data.pid_pit_speed_small_buff, POSITION_PID, 25000, 20000,
                    350.0f, 7.0f, 0); 
    PID_struct_init(&gimbal_data.pid_yaw_small_buff, POSITION_PID, 4, 0,
                    0, 0, 0);
    PID_struct_init(&gimbal_data.pid_yaw_speed_small_buff, POSITION_PID, 4, 25000,
                    0, 0, 0);
    ...
    PID_struct_init(&gimbal_data.pid_pit_speed_big_buff, POSITION_PID, 27000, 25000,
                    150.0f, 8.0f, 0); 
    PID_struct_init(&gimbal_data.pid_yaw_big_buff, POSITION_PID, 4, 0,
                    0, 0, 0); 
    PID_struct_init(&gimbal_data.pid_yaw_speed_big_buff, POSITION_PID, 4, 0,
                    0, 0, 0);
...
//		else if(RC_CtrlData.mouse.press_r&&gimbal_data.auto_aim_rotate_flag==1&&new_location.flag==1)//小陀螺自瞄
		if(gimbal_data.auto_aim_rotate_flag==1&&New_Auto_Aim.if_receive_data!=0)//小陀螺自瞄
		{
...
//                    gimbal_data.gim_ref_and_fdb.yaw_angle_ref = new_location.x + auto_aim_Yaw_remain;
                    ...
                    gimbal_data.gim_ref_and_fdb.yaw_angle_ref = New_Auto_Aim.Yaw_Angle + auto_aim_Yaw_remain;
