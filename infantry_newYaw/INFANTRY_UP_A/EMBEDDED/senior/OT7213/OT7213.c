...
void OT_Data_Process(CanRxMsg *msg,u8 ID,OT7213_t* _OT)
{
	if((msg->Data[0])==ID&&msg->DLC==8)
	{
		...
		//逆转方向
		_OT->OT_Pos=360-(float)(Pos_Medium)/1048576*360-123.808f;
...
/**********************
*@Brief:OT电机初始化增量模式及心跳信号发送
*@Call:内部或外部
*@Param:
*@Note:增量模式下的反馈才能用，只需要发送一次；
*      上位机一般设置1000ms不给电机发送启动命令就停止电流输出，因此要定时发送心跳信号
*@RetVal:无
**********************/

void OT_Init_And_Heartbeating(CAN_TypeDef *CANx,u8 ID,OT7213_t * _OT,OT_Special_Instruction_e _OT_Special_Instruction)
{	
	OT_Special_Instruction(CANx,ID,_OT_Special_Instruction);
	if(_OT->If_Inc_Init==0)
	{
		_OT->If_Inc_Init=1;
	}
}
